...
import { getNonvaluable } from '../../actions/items';
import Moment from 'react-moment';
import moment from 'moment';
import './NonvaluableToRelease.css';

const NonvaluableToRelease = ({
  getNonvaluable,
  ...
  return loading ? (
    ...
    <Fragment>
      ...
      <p className='lead'>
        <i className='fas fa-tshirt'></i> List of all lost and found
        non-valuable to released
      </p>

      <div className='nonvaluable-preview'>
            ...
            {lostFound.length > 0 && datefrom && dateto
              ? lostFound
                  ...
                  .map((item) => (
                    <Fragment key={item._id}>
                      ...
                      item.status1 ===
                        'to be claim' ? null : item.nonvaluable &&
              ...
              : lostFound.map((item) => (
                  <Fragment key={item._id}>
                    ...
                    item.status === 'to be claim' ? null : item.nonvaluable &&
...
NonvaluableToRelease.propTypes = {
  getNonvaluable: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, { getNonvaluable })(
  NonvaluableToRelease
);
